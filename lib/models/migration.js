...
module.exports = function(Migration, options) {
  ...
  Migration.migrate = function(upOrDown, since, options, cb) {
    if (cb === undefined) {
    ...
    assert(typeof since === 'string', 'The since argument must be a string, not ' + typeof since);
    ...
    Migration.findScriptsToRun(upOrDown, since, function runScripts(err, scriptsToRun) {
  ...
  Migration.findScriptsToRun = function(upOrDown, since, cb) {
    cb = cb || utils.createPromiseCallback();
    ...
    Migration.find(filters)
      .then(function(scriptsRun) {
        if (upOrDown === 'up') {
          ...
          // filter out those that come after the requested since value.
          if (since) {
            candidateScripts = candidateScripts.filter(function(fileName) {
              var name = fileName.slice(0, -3);
              debug('checking wether to run %s (%s <= %s)', name, name, since);
              var inRange = name <= since;
          ...
          cb(null, scriptsRun.map(Migration.mapScriptObjName));
        }
      })
      .catch(function(err) {
